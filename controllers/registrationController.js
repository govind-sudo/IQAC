...
// // Field names -> where in req.files (multer .fields()) they land, and
// // how to read the matching percentage/qualKey out of the already-parsed
// // body. Mirrors the client-side percentageFor() logic in
// // public/js/fileVerification.js, but this copy is the one that's
// // actually trusted.
// function percentageForField(fieldName, body, qualKey) {
//   if (fieldName === 'education[tenth][marksheet]') {
//     return body.education?.tenth?.percentage;
//   }
//   if (fieldName === 'education[twelfth][marksheet]' || fieldName === 'education[diploma][marksheet]') {
//     return body.education?.[qualKey]?.percentage;
//   }
//   return undefined;
// }

// /**
//  * Re-runs OCR + rule/AI verification for every file that was actually
//  * uploaded, server-side, using the trusted req.body values rather than
//  * whatever the client claimed in its earlier AJAX calls. This is the
//  * authoritative check — the AJAX endpoints in fileVerificationController.js
//  * exist only to give the student fast feedback and are never sufficient
//  * on their own, since they're trivially bypassable by posting directly
//  * to /register.
//  *
//  * @returns {Promise<Array<{ fieldName: string, mismatches: Array }>>}
//  *          list of fields that failed verification (empty if all passed)
//  */
// async function verifyAllUploadedFiles(req, body, qualKey) {
//   const { validateFileSignature } = require('../utils/fileSignatureValidator');
//   const { NO_VERIFICATION_FIELDS } = require('../utils/documentVerifier');
...
//     });
//     if (!sig.valid) {
//       failures.push({ fieldName, mismatches: [{ type: 'file', message: sig.reason }] });
//       continue;
//     }

//     // Skip OCR/AI entirely for fields that don't need it — e.g. caste
//     // certificate. Signature check above already confirmed it's a
//     // real, valid file; nothing further to verify.
...
//     if (!result.verified) {
//       failures.push({ fieldName, mismatches: result.mismatches });
//     }
//   }
...
// function friendlyFieldLabel(fieldName) {
//   const map = {
//     'education[tenth][marksheet]': '10th Marksheet',
//     'education[twelfth][marksheet]': '12th Marksheet',
//     'education[diploma][marksheet]': 'Diploma Marksheet',
//     'documents[aadhaarProof]': 'Aadhaar Proof',
//     'documents[abcIdProof]': 'ABC ID Proof',
//     'documents[casteProof]': 'Caste Proof',
//     'documents[pwdProof]': 'PWD Proof',
//     'documents[leavingCertificate]': 'Leaving Certificate',
//     'documents[aoLevelCertificate]': 'A/O Level Certificate',
//     'documents[puAdmissionLetter]': 'PU Offer Letter',
//     'documents[passportUpload]': 'Passport',
//   };
...
//       }
//     } else {
//       // International students: education details are optional on the
//       // form, but if they supplied a schoolName they must also supply a
//       // percentage (and vice versa) — cheap sanity check, not a hard block.
//       for (const level of ['tenth', 'twelfth', 'diploma']) {
//         const entry = body.education?.[level];
//         if (entry && (entry.schoolName) !== (entry.percentage) && (!entry.schoolName || !entry.percentage)) {
//           // one supplied without the other — ignore silently rather than
//           // blocking submission; international education requirements
//           // vary too much to hard-require this.
//         }
//       }
//     }
...
//     // ---------- 2b. Authoritative server-side document verification ----------
//     // The pre-submit AJAX checks in fileVerificationController.js are a
//     // UX convenience only and are trivially bypassable by posting
//     // directly to this route. Re-run OCR + rule/AI verification here,
//     // against the trusted req.body values, before anything is written to
//     // disk or saved to the database.
//     // const verificationFailures = await verifyAllUploadedFiles(req, body, qualKey);
//     // if (verificationFailures.length) {
//     //   const messages = verificationFailures.map((f) => {
//     //     const firstMsg = f.mismatches?.[0]?.message || 'could not be verified';
//     //     return `${friendlyFieldLabel(f.fieldName)}: ${firstMsg}`;
//     //   });
//     //   return res.status(400).render('students/register', {
//     //     error: `Document verification failed — ${messages.join(' ')}`,
//     //   });
//     // }

//     // ---------- 2c. Persist uploaded files (Storage Service) ----------
//     // Signature Validator (middleware) and the verification pass above
//     // have both approved every buffer in req.files, and all field/
//     // business-rule checks above have passed, so it's now safe to write
//     // them to disk with UUID names and fold the resulting paths back
//     // into `body` at the same nested keys the rest of this function
//     // already expects (documents.xxx, education.tenth.marksheet,
//     // education[qualKey].marksheet, etc).
...
//     // ---------- 7. Education block ----------
//     // Indian students: only the active 12th/diploma key + 10th, as before.
//     // International students: persist whatever they optionally filled in
